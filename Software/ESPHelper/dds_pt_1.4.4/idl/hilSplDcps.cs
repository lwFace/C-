...
namespace datarouter
{
    ...
    #region HIL_msgMarshaler
    public sealed class HIL_msgMarshaler : DDS.OpenSplice.CustomMarshalers.DatabaseMarshaler
    {
        private static readonly Type type = typeof(__HIL_msg);
        ...
        {
            HIL_msg from = untypedFrom as HIL_msg;
    ...
    #region __HIL_Channel
    [StructLayout(LayoutKind.Sequential)]
    public sealed class __HIL_Channel
    {
        public int id;
        public IntPtr msgs;
    }
    ...
    #region HIL_ChannelMarshaler
    public sealed class HIL_ChannelMarshaler : DDS.OpenSplice.CustomMarshalers.DatabaseMarshaler
    {
        private static readonly Type type = typeof(__HIL_Channel);
        public static readonly int Size = Marshal.SizeOf(type);
        ...
        public override void InitEmbeddedMarshalers(IDomainParticipant participant)
        {
            if (attr1Marshaler == null) {
                attr1Marshaler = DatabaseMarshaler.GetMarshaler(participant, typeof(datarouter.HIL_msg));
                if (attr1Marshaler == null) {
                    attr1Marshaler = new datarouter.HIL_msgMarshaler();
                    DatabaseMarshaler.Add(participant, typeof(datarouter.HIL_msg), attr1Marshaler);
        ...
        public override bool CopyIn(System.IntPtr basePtr, object untypedFrom, System.IntPtr to, int offset)
        {
            HIL_Channel from = untypedFrom as HIL_Channel;
            if (from == null) return false;
            Write(to, offset + offset_id, from.id);
            ...
            if (attr1Seq0Type == IntPtr.Zero) {
                attr1Seq0Type = DDS.OpenSplice.Database.c.resolve(basePtr, "datarouter::HIL_Channel::C_SEQUENCE<datarouter::HIL_msg>");
            }
        ...
        public override void CopyOut(System.IntPtr from, ref object to, int offset)
        {
            HIL_Channel dataTo = to as HIL_Channel;
            if (dataTo == null) {
                dataTo = new HIL_Channel();
                to = dataTo;
            }
            dataTo.id = ReadUInt32(from, offset + offset_id);
            int attr1Cursor0 = offset + offset_msgs;
            ...
            if (dataTo.msgs == null || dataTo.msgs.Length != attr1Seq0Length) {
                datarouter.HIL_msg[] target = new datarouter.HIL_msg[attr1Seq0Length];
                initObjectSeq(dataTo.msgs, target);
                dataTo.msgs = target as datarouter.HIL_msg[];
            }
            attr1Cursor0 += 4;
            int attr1Cursor1 = 0;
            for (int i0 = 0; i0 < attr1Seq0Length; i0++) {
                ...
                if (dataTo.msgs[i0] == null) {
                    dataTo.msgs[i0] = elementObj as datarouter.HIL_msg;
                }
                attr1Cursor1 += 16;
            }
        }
